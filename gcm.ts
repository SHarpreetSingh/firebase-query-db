...
const AdnMessaging = admin.messaging();

class messagingFirebaseHelper {
  messaging() {
    const payload = {
      notification: {
    ...
    AdnMessaging.send(payload)
      .then((result: any) => {
        console.log(result);
      })
      .catch((e: any) => console.log("err uncp=>", e));
  }
}

const mFire = new messagingFirebaseHelper();
mFire.messaging();
